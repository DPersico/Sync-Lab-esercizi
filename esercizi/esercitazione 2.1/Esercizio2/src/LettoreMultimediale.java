...
public class LettoreMultimediale {
	...
	if(daInserire.equalsIgnoreCase("filmato")) {
		...
		System.out.println("(Deve essere un valore intero)");
		try {
		luminositaInput = tastiera.nextInt(); }
		catch (InputMismatchException e) {
		tastiera.nextLine();
		System.out.println("Non hai inserito un valore intero");
		ok = false;
		...
		elementi[i] = new Filmato(titoloInput, durataInput, volumeInput, luminositaInput);
	...
	if(daInserire.equalsIgnoreCase("immagine")) {
		...
		elementi[i] = new Immagine(titoloInput, luminositaInput);
	...
	while(ok) {
		...
		while(!(scelta2.equalsIgnoreCase("si") || (scelta2.equalsIgnoreCase("no")))) {
			System.out.println("Vuoi modificare la luminosita? ");
		...
		if(scelta2.equalsIgnoreCase("si")) {
			System.out.println("Digita:  +luminosita | -luminosita");
			modifica1 = tastiera.next();
			if(modifica1.equalsIgnoreCase("+luminosita")) {
			elementi[input - 1].piuLuminosita();
			elementi[input - 1].apri();
			}else if(modifica1.equalsIgnoreCase("-luminosita")) {
				elementi[input - 1].menoLuminosita();
				elementi[input-1].apri();
				}
			}
